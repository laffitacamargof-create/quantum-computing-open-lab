...
class QuantumSync {
    ...
    connectStudio(iframe) {
        ...
        window.addEventListener('message', async (event) => {
            ...
            if (data.type === 'QCOL_APP_DELETED') {
                if (data.appId) {
                    try {
                        await this.deleteAppEverywhere(data.appId);
                    } catch (error) {
                        console.warn('⚠️ Could not delete app from cloud:', error.message);
                    }
                }
            }
    ...
    async deleteAppEverywhere(appId) {
        // A project deleted from App Studio may currently live in either
        // table depending on whether it was ever approved. Deleting a
        // non-matching id is a no-op in Supabase (0 rows affected, no
        // error), so it's safe to try both unconditionally.
        const results = await Promise.allSettled([
            this._supabaseDelete(this.TABLES.PENDING_APPS, appId),
            this._supabaseDelete(this.TABLES.PUBLISHED_APPS, appId)
        ]);
        await this.syncAll();
        const failed = results.filter(r => r.status === 'rejected');
        if (failed.length === results.length) {
            throw new Error(failed[0].reason?.message || 'Delete failed on both tables');
        }
        return true;
    }
